...
public class WeatherService {
	...
	public WeatherService(RestClient restClient, @Value("${openweathermap.api-key}") String apiKey) {
	...
	public Root getWeather(double lat, double lon) {
		try {
			...
			throw new ResponseStatusException(HttpStatus.BAD_GATEWAY,
					"Не удалось подключиться к OpenWeatherMap. Проверьте интернет-соединение и повторите запрос.");
		}
	}
