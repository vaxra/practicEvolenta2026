...
public class LocationService {
	...
	public List<Location> getAll(String name) {
		if (name != null) {
	...
	public LocationUpdateResult update(String name, Location location) {
		return repository.findByName(name)
				.map(existing -> {
					existing.setLatitude(location.getLatitude());
					existing.setLongitude(location.getLongitude());
					return new LocationUpdateResult(repository.save(existing), false);
				})
				.orElseGet(() -> {
					location.setName(name);
					return new LocationUpdateResult(repository.save(location), true);
				});
	}
	...
	public record LocationUpdateResult(Location location, boolean created) {
	}
